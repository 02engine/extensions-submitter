...
2. **存储**：数据库文件 `tokens.db` 存放在独立仓库中（环境变量 `TOKEN_REPO` 指定）。数据库以**明文**形式保存 token，便于管理员直接查阅/找回凭证。每次读写时从 GitHub 下载 → 内存中用 sql.js 操作 → 整体写回仓库。
...
4. **安全门控**：查询与更新接口均受 Cap 人机验证保护；更新接口额外校验凭证。
